...
| Markdown escaped where CPSC text enters `content[]` | Rendered as CommonMark, 43 records lost characters. Literal `*` in serial and model patterns became emphasis in 41 of them and dropped the asterisks. The tools backslash-escape CPSC text as they interpolate it, and leave a bare web address GFM autolinks as it is, because an autolink shows a backslash inside it literally; an address GFM leaves as plain text (a `www.` address after punctuation such as `"`, or a host with `_` in its last two labels) is escaped like the text around it. A carriage return becomes a line feed first, since CommonMark ends a line at either and an unescaped marker after one would open a block outside the quote. After that, every text field of every record renders unaltered under CommonMark and GFM. `cpsc_url` goes through as a percent-encoded link destination, since seven CPSC recall URLs contain spaces, with any backslash or character reference in it backslash-escaped so the destination is not decoded into a different address. Image and coordinated-recall addresses render bare, encoded the same way: 1,282 image URLs across 420 records contain spaces, and nine end in `.` or `_`, which GFM leaves out of an autolink, so that final character is percent-encoded too. Each rendered link then resolves to its stored address. An image entry that is not a web address (26796 holds a sentence) is escaped as text instead. The list tools escape inside `renderRecallBlock`, so the page budget charges the escaped bytes (up to 56 more on one search block). `structuredContent` stays unescaped. |
